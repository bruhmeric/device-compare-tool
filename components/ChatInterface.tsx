...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ history, onSendMessage, isTyping, isLoading }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-10 bg-slate-800 rounded-2xl shadow-2xl shadow-slate-950/50 border border-slate-700 flex flex-col h-[70vh] animate-fade-in">
      ...
      <div className="flex-1 p-6 overflow-y-auto space-y-4">
        {history.map((msg, index) => (
          <div key={index} className={`flex items-end gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {msg.role === 'assistant' && <CpuChipIcon className="w-8 h-8 p-1.5 rounded-full bg-cyan-500 text-slate-900 flex-shrink-0" />}
        ...
        {isTyping && (
          <div className="flex items-end gap-3 justify-start">
             <CpuChipIcon className="w-8 h-8 p-1.5 rounded-full bg-cyan-500 text-slate-900 flex-shrink-0" />
      ...
      <form onSubmit={handleSend} className="p-4 border-t border-slate-700 flex items-center gap-4">
        <input
          type="text"
          value={input}
